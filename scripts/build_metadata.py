...
class GitBuildMetadata:
    ...
    def __init__(self, repository_root: Path):
        ...
        self._commit_cache: dict[tuple[str, ...], str] = {}
    ...
    def last_modified_commit(self, *source_paths: str | Path) -> str:
        relative_paths = tuple(
            sorted({self._relative_path(path) for path in source_paths})
        )
        if not relative_paths:
            raise ValueError("At least one source path is required")
        if relative_paths not in self._commit_cache:
            self._commit_cache[relative_paths] = self._read_git_value(
                relative_paths, "%H", "commit"
            )
        return self._commit_cache[relative_paths]
    ...
    def _read_git_date(self, relative_paths: tuple[str, ...]) -> str:
        value = self._read_git_value(relative_paths, "%cs", "date")
        try:
    ...
    def _read_git_value(
        self, relative_paths: tuple[str, ...], format_value: str, label: str
    ) -> str:
        try:
            result = subprocess.run(
                ["git", "log", "-1", f"--format={format_value}", "--", *relative_paths],
        ...
        if result.returncode != 0 or not value:
            ...
            raise BuildMetadataError(
                f"Unable to determine a Git {label} for {paths}: {detail}"
            )
        return value
